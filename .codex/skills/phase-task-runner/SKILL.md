...
description: Run one active OKF phase task at a time in this repository. Use when the user asks to do the next task, continue phase work, create or update .okf/phase task files, inspect PHASE_SUMMARY.md, mark a task done, or propose the next PPP_TTT task without pre-creating future tasks.
---
...
- Each `PPP_TTT` task is one implementation step inside that phase, with its
...
- Load `.okf/phase/<PPP>/PHASE_SUMMARY.md` before opening task files.
- Work on only one task file at a time.
- Use task ids in `PPP_TTT` format, for example `001_002`.
- Keep only the active phase broken into task files.
...
- Do not create future task files unless the user explicitly asks.
- After finishing a task, update the task file and `PHASE_SUMMARY.md` in the same pass.
...
3. Read only that task file plus directly relevant source files.
...
Every phase summary and task file must have YAML frontmatter.
